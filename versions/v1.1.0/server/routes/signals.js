...
router.get('/daily-summary', (req, res) => {
  ...
  // Stance changes: compare today vs yesterday dominant stance per person
  ...
  function getDominant(rows) {
    ...
    for (const [person, stances] of Object.entries(map)) {
      ...
      if (entries.length > 0) {
        result[person] = entries.sort((a, b) => b[1] - a[1])[0][0];
      }
    }
    return result;
  }
  ...
  // Person daily summaries (comprehensive analysis)
  const personDailySummaries = db.prepare(`
    SELECT dps.*, p.name as person_name
    FROM daily_person_summaries dps
    JOIN people p ON dps.person_id = p.id
    WHERE dps.date = ?
  `).all(targetDate);
