...
import { Plus, X, Pencil, Trash2, User, Users } from 'lucide-react'
    ...
    <div>
      ...
      <div className="mb-6 rounded-lg border px-4 py-3.5" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg-secondary)' }}>
        ...
        <p className="text-sm leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
          You follow <strong style={{ color: 'var(--color-text-primary)' }}>Dylan Patel</strong> (SemiAnalysis). Create a person with name "Dylan Patel" and tags "semiconductor, chips, AI". Then link his Twitter, YouTube, and Substack sources. All his content across platforms will be grouped here.
        </p>
      </div>

      {loading ? <Skeleton lines={5} /> : people.length === 0 ? (
        <EmptyState
          icon={Users}
          title="No people yet"
          description="Add the first person to start tracking their signals."
          action={
            <button
              onClick={() => { setEditing(null); setForm({ name: '', bio: '', tags: '' }); setShowForm(true) }}
