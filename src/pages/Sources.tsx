...
import { Plus, X, Pencil, Trash2, RefreshCw, Radio } from 'lucide-react'

const SOURCE_TYPES = [
  { value: 'substack_rss', label: 'Substack RSS', fields: [{ key: 'feed_url', label: 'Feed URL', placeholder: 'https://newsletter.xxx.com/feed' }] },
  { value: 'twitter', label: 'Twitter', fields: [
    { key: 'username', label: 'Username', placeholder: 'daborm' },
    { key: 'bearer_token', label: 'Bearer Token', placeholder: 'AAAAAAAA...' },
  ] },
  { value: 'reddit_rss', label: 'Reddit RSS', fields: [{ key: 'subreddit', label: 'Subreddit', placeholder: 'SemiAnalysis' }] },
  { value: 'youtube_rss', label: 'YouTube RSS', fields: [{ key: 'channel_id', label: 'Channel ID', placeholder: 'UC...' }] },
]
    ...
    <div>
      ...
      <div className="mb-6 rounded-lg border px-4 py-3.5" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg-secondary)' }}>
        ...
        <p className="text-sm leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
          For <strong style={{ color: 'var(--color-text-primary)' }}>Dylan Patel</strong>, add 4 sources linked to him:
        </p>
        <ul className="mt-1.5 ml-4 space-y-0.5 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
          <li>Twitter — username <code className="px-1 py-0.5 rounded text-xs font-mono" style={{ background: 'var(--color-bg-tertiary)' }}>dylan522p</code>, interval 30 min</li>
          <li>YouTube — channel ID, interval 1 hour</li>
          <li>Substack — SemiAnalysis RSS URL, interval 2 hours</li>
          <li>Reddit — subreddit <code className="px-1 py-0.5 rounded text-xs font-mono" style={{ background: 'var(--color-bg-tertiary)' }}>SemiAnalysis</code>, interval 4 hours</li>
        </ul>
      </div>
      ...
      {loading ? <Skeleton lines={5} /> : sources.length === 0 ? (
        <EmptyState
          ...
          description="Add a source to start collecting signals from social media and blogs."
          action={
            <button
              onClick={() => { setEditing(null); setForm({ type: 'substack_rss', person_id: '', config: {}, poll_interval_min: 30 }); setShowForm(true) }}
        ...
        <div className="border rounded-lg overflow-hidden" style={{ borderColor: 'var(--color-border)' }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {sources.map(s => {
                const rawCfg = JSON.parse(s.config || '{}')
                // Mask sensitive fields in display
                const cfg = Object.fromEntries(
                  Object.entries(rawCfg).map(([k, v]) => [k, k.includes('token') || k.includes('key') || k.includes('secret') ? '••••••••' : v])
                )
                const displayValue = Object.values(cfg)[0] as string || '-'
